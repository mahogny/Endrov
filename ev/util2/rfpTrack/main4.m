...
[vox,zs]=getVoxStack(channel,2413);

colormap('gray');

%d=pl(10).getArrayImage;
...
amin=squeeze(min(shiftdim(vox,2)));
for z=1:length(zs)
    vox(:,:,z)=((vox(:,:,z)-am)*2);
end
for z=1:length(zs)
...
thresholdcolor=3; %10-20 at late stages
...
%Go to unscaled pixel coordinates
...
for i=1:size(maximas,1)
    ty=round(maximas(i,2));
    tx=round(maximas(i,1));
    tz=round(maximas(i,3));
    fim(ty,tx,tz)=256;
end

%clear fims;
%for z=1:length(zs)
%    fims(:,:,z)=imresize(vox(:,:,z),1/scalefactor);
%end
fims=im;
...
%for i=1:numcand
%    M(i)=im2frame(1+fims(:,:,i)/10,colormap);
%end
