...
public static void calcAP(File file, int fileNumber)
	...
	try
		...
		//if (currentpath.contains("NRed")) // only deal with these image sets
			...
			outFile.write(fileNumber + "\t" +currentostname+"\t");
			...
			for(int frame:images.keySet())
				...
				for(int z:zs.keySet())
					...
					for (int co=0;co<w;co++)
						for (int ro=0;ro<h;ro++)
							...
							double p = pix[0];											
							if (p>=40) // empirically determined threshold (background)
								{
								totalintens+=p;
								pixCounter++;
				...
				if (overexposed < 10)
					{	
					goodFrameCounter++;
					intermedFile.write(intensities[frameCounter] + "\t" + signalpixels[frameCounter] + "\t"+exptimeArray[frameCounter]+"\n");
					}
				else
					{
