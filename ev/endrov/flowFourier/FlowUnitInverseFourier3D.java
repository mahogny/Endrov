...
//		Object inImag=flow.getInputValue(this, exec, "inImag");
	...
	//	checkNotNull(inReal);
		...
		if(!inImag.hasValue())
			{
			AnyEvImage[] outs=new EvOpFourierRealInverseFull3D(true).execUntyped(exec.ph, inReal);
			lastOutput.put("outReal", outs[0]);
			lastOutput.put("outImag", outs[1]); //Could use half-inverse and generate special image?
			}
		else
			{
			AnyEvImage[] outs=new EvOpFourierComplexInverse3D(true).execUntyped(exec.ph, inReal, inImag.get());
			lastOutput.put("outReal", outs[0]);
