...
public class Lineage extends EvObject implements Cloneable
	...
	public void removeParticle(String name)
		...
		for(Particle n:particle.values())
			{
			n.child.remove(name);
			if(n.parent!=null && n.parent.equals(name))
				n.parent=null;
			}
		setMetadataModified();
		}
	...
	public void mergeParticles(String sourceName, String targetName)
		...
		if(theParentName!=null)
			...
			particle.get(theParentName).child.add(newName);
			}
	...
	public void associateParentChildCheckNoLoop(String parent, String child)
		...
		if(particle.get(child).parent!=null)
			...
			if(!checkForLoops(visited, child))
				...
				particle.get(child).parent=parent;
				}
			else
	...
	/**
	 * Remove parent reference from particle
	 */
	public void removeParentReference(String particleName)
		{
		...
		if(parentName!=null)
			{
			Particle parent=particle.get(parentName);
			parent.child.remove(particleName);
			}
		setMetadataModified();
		}
	...
	public class Particle implements Cloneable
		...
		public String toString()
			{
			return "parent "+parent+" children "+child;
			}
	...
	public void flattenSingleChildren()
		{
		for(String name:new LinkedList<String>(particle.keySet()))
			{
			Particle n=particle.get(name);
			if(n!=null)
				if(n.child.size()==1)
					...
					for(String childchildName:child.child)
						particle.get(childchildName).parent=name;
					}
			}
