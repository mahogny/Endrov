...
import endrov.gl.EvGLMeshVBO.MeshRenderSettings;
...
import endrov.typeLineage.Lineage.MeshRenderMode;
 ...
 */
public class LineageModelExtension implements Viewer3DWindowExtension
	{
  ...
  public void newModelWindow(Viewer3DWindow w)
	...
	static class NucModelWindowHook implements Viewer3DHook, ActionListener, Viewer3DMouseListener
		{
		...
		public NucModelWindowHook(final Viewer3DWindow w)
			{
			this.w=w;
		...
		public void actionPerformed(ActionEvent e)
			...
			if(e.getSource()==miShowSelectedNuc)
				{
				for(endrov.typeLineage.LineageSelParticle p:LineageCommonUI.getSelectedParticles())
				...
				{
				String inp=EvBasicWindow.showInputDialog("Enter magnification in percent", "100");
				if(inp!=null)
...
/*		private int displayListVisibleSphere;
      ...
      //gl.glColor3d(1,1,1);
//    	gl.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE, new float[]{colR,colG,colB}, 0);
			invokeDrawListVisible(gl);
      ...
      gl.glDisable(GL2.GL_LIGHTING);
		 ...
		 */
		private void renderParticleOverlay(GL2 gl, List<TransparentRenderer3D> transparentRenderers, LineageSelParticle nucPair, Lineage.InterpolatedParticle nuc, EvDecimal curFrame)
			{
      ...
      boolean shouldDrawExp=!expsettings.isEmpty() && !(showExpAs==ShowExp.CellColor || showExpAs==ShowExp.CellColorAND);
	    ...
	    if(shouldDrawText || shouldDrawExp)
	    	...
	    	if(shouldDrawText)
		    	...
		    	w.view.renderString(gl, transparentRenderers, (float)textScaleFactor, nucName, Color.BLUE);
    			gl.glTranslated(0, +displacement, 0);
	    		}
		...
		public Collection<BoundingBox3D> adjustScale()
			...
			for(Lineage lin:getVisibleLineages())
				for(String name:lin.particle.keySet())
					...
					if(mesh!=null)
						{
						BoundingBox3D v=mesh.getBoundingBox();
						if(v!=null)
							list.add(v);
						}
					}
	...
	static
		{
		Viewer3DWindow.modelWindowExtensions.add(new LineageModelExtension());
		}
