...
/**
 * Writes the pixels from a set of JPEG files to a single TIFF. The pixel data
 * is used as-is, so no decompression or re-compression is performed.
 * <dl>
 * <dt><b>Source code:</b></dt>
 * <dd><a href="http://trac.openmicroscopy.org.uk/ome/browser/bioformats.git/components/bio-formats/utils/WritePrecompressedPlanes.java"
 * >Trac</a>, <a href="http://git.openmicroscopy.org/?p=bioformats.git;a=blob;f=components/bio-formats/utils/WritePrecompressedPlanes.java;hb=HEAD"
 * >Gitweb</a></dd>
 * </dl>
 */
public class WritePrecompressedPlanes2
	{
	public static void main(String[] args) throws FormatException, IOException
		...
		TiffWriter twriter=new TiffWriter();
		twriter.setId(outputFile);
		
		twriter.setWriteSequentially(true);
		twriter.setBigTiff(false); //true possible?
...
//			twriter.saveBytes(planeID, buf, ifd, 0, 0, reader.getSizeX(), reader.getSizeY());
			...
			twriter.saveBytes(no, buf, ifd, x, y, w, h)
			
			
			//public void writeImage(byte[] buf, IFD ifd, int no, int pixelType, boolean last)
...
//boolean copyDirectly)
			...
			saver.writeImage(buf, ifd, i, pixelType, 0, 0, reader.getSizeX(), reader
					.getSizeY(), i==inputFiles.length-1, reader.getRGBChannelCount(),
					true);
		...
		for (int i = 0; i<inputFiles.length; i++)
			...
			saver.overwriteIFDValue(in, i, IFD.COMPRESSION, TiffCompression.JPEG
					.getCode());
			in.close();
			}
