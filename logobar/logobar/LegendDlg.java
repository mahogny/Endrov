...
class CMouseAdapter extends MouseAdapter{
    ...
    GroupColorPanel iGrpColor;
    public CMouseAdapter(GroupColorPanel grp){
	this.iGrpColor = grp;
    }
...
class GroupColorPanel extends JPanel{
    private int iIdx;
    public GroupColorPanel(int idx){
	super();
	setBorder(BorderFactory.createLineBorder(Color.black));
	addMouseListener( new CMouseAdapter(this));
	iIdx = idx;
    }
...
//	iParent = par;
	...
	ColorHandler cHandler = LogoBar.iColorHandler;
	System.out.println("Painting the legend pane");	
	for(int i = 0; i < 21; i++){
		...
		GroupColorPanel aColorFrame = new GroupColorPanel(i);
...
public class LegendDlg extends JDialog{
  
    private LegendPane iPane;
    public LegendDlg(JFrame frame) {
	super(frame, "Color Legend", false);
	setSize(100,400);
	...
	iPane = new LegendPane();
	setContentPane(iPane);
	repaint();
